...
```
### ผลการทดลอง
<img width="829" height="157" alt="image" src="https://github.com/user-attachments/assets/fc264e7e-4b0a-4be8-be56-ae09ce453d00" />
...
```
### ผลการทดลอง
<img width="216" height="365" alt="image" src="https://github.com/user-attachments/assets/2213b542-d8a4-4dea-8ff0-e7db1c3041ed" />
...
### ผลการทดลอง
```
<img width="804" height="156" alt="image" src="https://github.com/user-attachments/assets/2f5d0833-4f78-416b-8f73-6f960718dd19" />
...
```
1. รูปผลการทดลอง
2. อธิบายผลการทดลอง การ SELECT , INSERT, UPDATE, DELETE เป็นอย่างไร SELECT และ INSERT ทำงานเร็ว UPDATE และ DELETE ใช้เวลานาน → อาจต้องปรับ index หรือ logic ให้เบาลง
```
...
```
รูปผลการทดลอง
<img width="805" height="194" alt="image" src="https://github.com/user-attachments/assets/ac4ca6f9-7a68-44cf-b4ee-e9b39e2a4eef" />
...
```
### ผลการทดลอง
<img width="838" height="207" alt="image" src="https://github.com/user-attachments/assets/0cefebdc-2bc3-4597-9215-b80e8267f1f6" />
...
ORDER BY test_timestamp DESC;
```
### ผลการทดลอง SELECT config_name, test_type, shared_buffers_mb || 'MB' as shared_buffers, work_mem_mb || 'MB' as work_mem, ROUND(execution_time_ms, 2) as exec_time_ms, ROUND(buffer_hit_ratio, 2) as hit_ratio_percent, test_timestamp FROM benchmark_results ORDER BY test_timestamp DESC;
```
รูปผลการทดลอง
```
...
### ผลการทดลอง
```
รูปผลการทดลอง <img width="753" height="237" alt="image" src="https://github.com/user-attachments/assets/4952b81d-c4f9-482f-b888-47bb082d4c77" />
...
```
### ผลการทดลอง
  <img width="753" height="207" alt="image" src="https://github.com/user-attachments/assets/bf5a997a-8118-4d08-8c38-471a262fd1d1" />
...
## คำถามท้ายการทดลอง
1. หน่วยความจำใดบ้างที่เป็น shared memory และมีหลักในการตั้งค่าอย่างไร Shared Memory คือพื้นที่หน่วยความจำ (RAM) ส่วนกลางที่ PostgreSQL จองไว้เพื่อให้ทุกๆ การเชื่อมต่อ (Connection) และ Background Process ทั้งหมดสามารถเข้ามาใช้งานร่วมกันได้ 🧠. เปรียบเสมือนเป็น "ห้องสมุดกลาง" ของฐานข้อมูล
2. Work memory และ maintenance work memory คืออะไร มีหลักการในการกำหนดค่าอย่างไร Work Memory และ Maintenance Work Memory คืออะไร
work_mem (Work Memory): คือหน่วยความจำชั่วคราวที่ถูกจองให้ แต่ละ Operation (ไม่ใช่แต่ละ Connection) เพื่อใช้ในการทำงานที่ซับซ้อน เช่น การเรียงข้อมูล (Sorting), การรวมตาราง (Joining), และการทำ Hashing. เปรียบเสมือน "โต๊ะทดลองงานส่วนตัว" ที่แต่ละคำสั่งย่อยๆ จะมาใช้งาน

maintenance_work_mem: คือหน่วยความจำที่ใช้สำหรับงานบำรุงรักษาระบบฐานข้อมูลโดยเฉพาะ เช่น การสร้าง Index (CREATE INDEX), การล้างข้อมูลเก่า (VACUUM), และการเพิ่ม Foreign Key.
3. หากมี RAM 16GB และต้องการกำหนด connection = 200 ควรกำหนดค่า work memory และ maintenance work memory อย่างไร หากมี RAM 16GB, Connections = 200 ควรกำหนดค่าอย่างไร
นี่คือแนวทางการกำหนดค่าเบื้องต้นครับ:

shared_buffers: ตั้งไว้ที่ 25% ของ RAM ทั้งหมด

16 GB * 0.25 = 4 GB

กำหนดค่า: shared_buffers = 4GB

work_mem: ต้องคำนวณจาก worst-case scenario คือทุก connection ใช้งาน operation ที่ซับซ้อนพร้อมกันทั้งหมด (ซึ่งไม่ค่อยเกิดขึ้นจริง แต่ต้องเผื่อไว้)

เราต้องเหลือ RAM ให้ OS และส่วนอื่นๆ ด้วย สมมติว่าเหลือ RAM ให้ work_mem ประมาณ 4-6 GB


4. ไฟล์ postgresql.conf และ postgresql.auto.conf  มีความสัมพันธ์กันอย่างไร ความสัมพันธ์ระหว่าง postgresql.conf และ postgresql.auto.conf
postgresql.conf คือไฟล์ตั้งค่าหลักที่ผู้ดูแลแก้ไขเอง. ส่วน postgresql.auto.conf ถูกจัดการโดยคำสั่ง ALTER SYSTEM และค่าในไฟล์นี้จะ มีความสำคัญเหนือกว่า (override) ค่าที่ซ้ำกันใน postgresql.conf เสมอ.
5. Buffer hit ratio คืออะไร Buffer Hit Ratio
คือ เปอร์เซ็นต์ความสำเร็จในการค้นหาข้อมูลเจอในแคชของ RAM (shared_buffers) โดยไม่ต้องไปอ่านจากดิสก์ที่ช้ากว่า. ค่านี้ยิ่งสูง (ควรเกิน 99%) ยิ่งหมายถึงประสิทธิภาพที่ดี.
6. แสดงผลการคำนวณ การกำหนดค่าหน่วยความจำต่าง ๆ โดยอ้างอิงเครื่องของตนเอง ตัวอย่างการคำนวณค่าหน่วยความจำ
สำหรับเครื่องที่มี RAM 32GB, การตั้งค่าเริ่มต้นที่ดีอาจเป็น:
...
work_mem: 32MB
7. การสแกนของฐานข้อมูล PostgreSQL มีกี่แบบอะไรบ้าง เปรียบเทียบการสแกนแต่ละแบบ ประเภทการสแกนข้อมูล
Sequential Scan คือการอ่านข้อมูลทุกแถวในตาราง เหมาะกับตารางขนาดเล็ก.
Index Scan ใช้ดัชนีเพื่อชี้ไปยังแถวที่ต้องการโดยตรง เหมาะกับการดึงข้อมูลเฉพาะบางส่วนจากตารางขนาดใหญ่และเร็วกว่ามาก.
